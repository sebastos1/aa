...
impl Cache {
    ...
    async fn get_cached_or_fetch(&self, uuid: &str) -> Option<(String, String)> {
...
pub async fn init_profiles(cache: &Cache, players: &mut HashMap<String, crate::structs::Player>) {
    println!("[INIT] Initializing player profiles for {} players...", players.len());
    
    let mut fetch_count = 0;
    let mut cache_hits = 0;

    for (uuid, player) in players.iter_mut() {
        if let Some((name, avatar_url)) = cache.get_cached_or_fetch(uuid).await {
    ...
    println!("[INIT] Profile initialization complete. Cache hits: {}, API fetches: {}", cache_hits, fetch_count);
}
...
pub fn cache_world_icon(world_path: &StdPath) -> Result<Option<String>> {
    ...
    if source_icon_path.exists() {
        ...
        return Ok(Some(format!("/{}/world/icon.png", CACHE_DIR)));
    }
