...
use crate::load::{context::{LoadingContext, tag_lookup}, strip_mc_prefix};
...
fn expand_id(id: &str, context: &LoadingContext) -> Vec<String> {
    if id.starts_with('#') {
        tag_lookup(&context.tag_map, id, &mut HashSet::new())
    } else {
        vec![strip_mc_prefix(id).to_string()]
    }
}

fn extract_ids(value: &serde_json::Value, context: &LoadingContext) -> Vec<String> {
    match value {
        serde_json::Value::String(s) => expand_id(s, context),
        serde_json::Value::Array(arr) => {
            arr.iter()
                .filter_map(|v| v.as_str())
                .flat_map(|s| expand_id(s, context))
                .collect()
        },
        _ => Vec::new(),
    }
...
fn get_ids_from_fields(obj: &serde_json::Value, fields: &[&str], context: &LoadingContext) -> Vec<String> {
    for field in fields {
        if let Some(value) = obj.get(field) {
            let ids = extract_ids(value, context);
            if !ids.is_empty() {
                return ids;
            }
        }
...
fn create_item_subject(ids: Vec<String>, count: Option<i32>, variant: Option<String>, custom_name: Option<String>) -> Subject {
    Subject {
        base: BaseSubject::Item { ids, count, variant, custom_name },
        supplements: Vec::new(),
    }
}

fn create_entity_subject(id: String, variant: Option<String>, custom_name: Option<String>) -> Subject {
    Subject {
        base: BaseSubject::Entity { id, variant, custom_name },
        supplements: Vec::new(),
    }
}
...
pub fn extract_subjects(conditions: &serde_json::Value, trigger: Option<&str>, context: &LoadingContext) -> Vec<Subject> {
    ...
    for (key, value) in obj {
        match key.as_str() {
            "items" | "item" | "blocks" | "block" | "fired_from_weapon" => subjects.extend(extract_items(value, context)),
            "entity" | "source" | "cause" | "bystander" | "lightning" | "victims" | 
            "parent" | "partner" | "child" | "projectile" => subjects.extend(extract_entities(value, context)),
            "villager" => subjects.extend(extract_villagers(value, context)),
            "player" => subjects.extend(extract_players(value, context)),
            "damage" | "killing_blow" => subjects.extend(extract_damage(value, context)),
            "effects" => subjects.extend(extract_effects(value)),
            "potion" => subjects.extend(extract_stored_potions(value)),
            "location" => subjects.extend(extract_location(value, context)),
            "recipe_id" => subjects.extend(extract_recipes(value, context)),
            "loot_table" => subjects.extend(extract_loot_tables(value)),
            "advancement" => subjects.extend(extract_advancement_conditions(value)),
            _ => {}
        }
    }
...
fn extract_trigger_subjects(trigger: &str) -> Vec<Subject> {
    match strip_mc_prefix(trigger) {
        "fishing_rod_hooked" => vec![create_item_subject(vec!["fishing_rod".to_string()], None, None, None)],
        "cured_zombie_villager" => vec![create_entity_subject("zombie_villager".to_string(), None, None)],
        _ => Vec::new(),
    }
}
...
fn extract_items(value: &serde_json::Value, context: &LoadingContext) -> Vec<Subject> {
    ensure_array(value).into_iter().filter_map(|obj| extract_single_item(obj, context)).collect()
}

fn extract_single_item(item_obj: &serde_json::Value, context: &LoadingContext) -> Option<Subject> {
    let mut item_ids = get_ids_from_fields(item_obj, &["items", "item"], context);
...
fn extract_item_properties(item_obj: &serde_json::Value) -> (Option<String>, Option<String>) {
    ...
    if custom_name.is_none() {
        if let Some(predicates) = item_obj.get("predicates") {
            ...
            if variant.is_none() {
                variant = extract_firework_variant(predicates);
            }
        }
    }
...
fn extract_firework_variant(predicates: &serde_json::Value) -> Option<String> {
    if let Some(fireworks) = predicates.get("minecraft:fireworks") {
        if let Some(explosions) = fireworks.get("explosions") {
            if let Some(contains) = explosions.get("contains").and_then(|v| v.as_array()) {
                for explosion in contains {
                    if let Some(shape) = explosion.get("shape").and_then(|v| v.as_str()) {
...
fn extract_item_components(components: &serde_json::Value) -> Vec<Supplement> {
    ...
    // Handle dyed color
    if let Some(dyed_color) = components.get("minecraft:dyed_color").and_then(|v| v.as_i64()) {
        if let Some(color_name) = get_dye_color_name(dyed_color as i32) {
            supplements.push(Supplement::DyeColor { 
                color: color_name,
                decimal_value: dyed_color as i32
            });
        }
    }
...
fn get_dye_color_name(decimal_value: i32) -> Option<String> {
    match decimal_value {
        16383998 | 15790320 => Some("white".to_string()),    // JE & BE white
        16351261 => Some("orange".to_string()),
        13061821 => Some("magenta".to_string()),
        3847130 => Some("light_blue".to_string()),
        16701501 => Some("yellow".to_string()),
        8439583 => Some("lime".to_string()),
        15961002 => Some("pink".to_string()),
        4673362 => Some("gray".to_string()),
        10329495 => Some("light_gray".to_string()),
        1481884 => Some("cyan".to_string()),
        8991416 => Some("purple".to_string()),
        3949738 => Some("blue".to_string()),
        8606770 => Some("brown".to_string()),
        6192150 => Some("green".to_string()),
        11546150 => Some("red".to_string()),
        1908001 => Some("black".to_string()),
        _ => None,
    }
}
...
fn find_entity_types(value: &serde_json::Value, context: &LoadingContext) -> Vec<String> {
    if let serde_json::Value::Object(obj) = value {
        if let Some(type_value) = obj.get("type") {
            let types = extract_ids(type_value, context)
                .into_iter()
...
fn extract_players(value: &serde_json::Value, context: &LoadingContext) -> Vec<Subject> {
    ...
    for player_obj in ensure_array(value) {
        ...
        subjects.extend(extract_player_context(predicate, context));
...
fn extract_player_context(predicate: &serde_json::Value, context: &LoadingContext) -> Vec<Subject> {
    let mut subjects = Vec::new();
    ...
    if let Some(stepping_on) = predicate.get("stepping_on") {
        if let Some(block_data) = stepping_on.get("block") {
            if let Some(block_str) = block_data.as_str() {
                let block_ids = expand_id(block_str, context);
                subjects.push(create_item_subject(block_ids, None, None, None));
            } else if let Some(blocks) = block_data.get("blocks") {
                let block_ids = extract_ids(blocks, context);
                if !block_ids.is_empty() {
                    let variant = extract_block_state_variant(block_data);
                    subjects.push(create_item_subject(block_ids, None, variant, None));
                }
            }
        }
    }
...
fn extract_location(value: &serde_json::Value, context: &LoadingContext) -> Vec<Subject> {
    ...
    for location_obj in ensure_array(value) {
        if let Some(block_str) = location_obj.get("block").and_then(|v| v.as_str()) {
            let block_ids = expand_id(block_str, context);
            subjects.push(create_item_subject(block_ids, None, None, None));
        }
        ...
        match condition {
            ...
            Some("minecraft:inverted") => continue,
            _ => {
                subjects.extend(extract_location_predicate(predicate, context));
            }
        }
...
fn extract_location_predicate(predicate: &serde_json::Value, context: &LoadingContext) -> Vec<Subject> {
    ...
    if let Some(fluid_data) = predicate.get("fluid") {
        if let Some(fluids) = fluid_data.get("fluids") {
            let fluid_ids = extract_ids(fluids, context);
            if !fluid_ids.is_empty() {
                subjects.push(create_item_subject(fluid_ids, None, None, None));
            }
        }
    }
...
fn extract_location_components(predicate: &serde_json::Value, context: &LoadingContext) -> Vec<Subject> {
    ...
    if let Some(biome_value) = predicate.get("biomes") {
        biomes.extend(extract_ids(biome_value, context));
    }
    if let Some(structure_value) = predicate.get("structures") {
        structures.extend(extract_ids(structure_value, context));
    }
...
fn extract_location_terms(terms: &[serde_json::Value], context: &LoadingContext) -> Vec<Subject> {
    ...
    for term in terms {
        if let Some(block_str) = term.get("block").and_then(|v| v.as_str()) {
            let block_ids = expand_id(block_str, context);
            subjects.push(create_item_subject(block_ids, None, None, None));
        }
...
fn extract_loot_tables(value: &serde_json::Value) -> Vec<Subject> {
    if let Some(loot_table_id) = value.as_str() {
        vec![create_item_subject(vec!["chest".to_string()], None, Some(loot_table_id.to_string()), None)]
    } else {
        Vec::new()
    }
}
            ...
            for captures in regex.captures_iter(nbt) {
                if let Some(item_id) = captures.get(1) {
                    subjects.push(create_item_subject(
                        vec![strip_mc_prefix(item_id.as_str()).to_string()], 
                        None, 
                        None, 
                        None
                    ));
            ...
            if let Some(regex) = regex::Regex::new(pattern).ok() {
                if let Some(captures) = regex.captures(nbt) {
                    if let Some(block_id) = captures.get(1) {
                        subjects.push(create_item_subject(vec![strip_mc_prefix(block_id.as_str()).to_string()], None, None, None));
                        break;
                    }
                }
            }
...
fn deduplicate_subjects(subjects: Vec<Subject>) -> Vec<Subject> {
    let mut seen = HashSet::new();
    let mut unique_subjects = Vec::new();
    
    for subject in subjects {
        let subject_key = format!("{:?}", subject); // unique
        if !seen.contains(&subject_key) {
            seen.insert(subject_key);
...
fn extract_common_subjects(
    requirements_map: &mut BTreeMap<String, Vec<Subject>>,
    requirement_groups: &Option<Vec<Vec<String>>>
) -> Option<Vec<Subject>> {
    // Apply Python rules: more than 1 criteria AND not exactly 1 requirement group
    if requirements_map.len() <= 1 {
        return None;
    }
    
    if let Some(groups) = requirement_groups {
        if groups.len() == 1 {
            return None;
        }
    }
    
    // Get all criteria that have subjects
    let criteria_with_subjects: BTreeMap<String, &Vec<Subject>> = requirements_map
        .iter()
        .filter(|(_, subjects)| !subjects.is_empty())
        .map(|(name, subjects)| (name.clone(), subjects))
        .collect();
    
    // Need at least 2 criteria with subjects to find commons
    if criteria_with_subjects.len() < 2 {
        return None;
    }
    
    // Track which criteria each subject appears in
    let mut subject_appearances: HashMap<String, HashSet<String>> = HashMap::new();
    let all_criteria_names: HashSet<String> = criteria_with_subjects.keys().cloned().collect();
    
    for (criteria_name, subjects) in &criteria_with_subjects {
        for subject in subjects.iter() {
            let subject_hash = format!("{:?}", subject);
            subject_appearances
                .entry(subject_hash)
                .or_insert_with(HashSet::new)
                .insert(criteria_name.clone());
        }
    }
    
    // Find subjects that appear in ALL criteria
    let common_subject_hashes: Vec<String> = subject_appearances
        .iter()
        .filter(|(_, appearing_criteria)| **appearing_criteria == all_criteria_names)
        .map(|(hash, _)| hash.clone())
        .collect();
    
    if common_subject_hashes.is_empty() {
        return None;
    }
    
    // Extract one copy of each common subject
    let mut common_subjects = Vec::new();
    let common_hashes_set: HashSet<String> = common_subject_hashes.into_iter().collect();
    
    // Get the actual subject objects (from the first criteria)
    if let Some((_, first_subjects)) = criteria_with_subjects.iter().next() {
        for subject in first_subjects.iter() {
            let subject_hash = format!("{:?}", subject);
            if common_hashes_set.contains(&subject_hash) {
                common_subjects.push(subject.clone());
            }
        }
    }
    
    // Remove common subjects from individual criteria
    let criteria_names: Vec<String> = criteria_with_subjects.keys().cloned().collect();
    for criteria_name in criteria_names {
        if let Some(subjects) = requirements_map.get_mut(&criteria_name) {
            subjects.retain(|subject| {
                let subject_hash = format!("{:?}", subject);
                !common_hashes_set.contains(&subject_hash)
            });
        }
    }
    
    Some(common_subjects)
}
...
pub fn get_requirements(
...
) -> (BTreeMap<String, Vec<Subject>>, Option<Vec<Subject>>) {
    ...
    for (key, criterion) in criteria_json {
        ...
        let deduplicated = deduplicate_subjects(subjects);
        if !deduplicated.is_empty() {
            requirements_map.insert(strip_mc_prefix(key).to_string(), deduplicated);
        }
    }
    
    // Extract common subjects
    let common_subjects = extract_common_subjects(&mut requirements_map, &requirement_groups);
    ...
    let debug_info = AdvancementDebugInfo {
        ...
        common_subjects: common_subjects.clone(),
    };
    ...
    (requirements_map, common_subjects)
}
...
struct AdvancementDebugInfo {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    common_subjects: Option<Vec<Subject>>,
}
